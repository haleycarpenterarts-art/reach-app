...
### 2026-08-22 — RLS enforcement via SET LOCAL ROLE, not a second login role

**Decision:** Tenant-scoped work runs inside a transaction that sets `app.tenant_id` and `app.identity_id` and then issues `SET LOCAL ROLE app_user` — a NOLOGIN, NOBYPASSRLS role. All three are transaction-local, so a pooled connection returns to its normal role and an empty context on commit or rollback. `lib/prisma.ts` exposes this as `withTenant()`; the bare `prisma` client stays privileged and is for platform-level work only.
**Context:** The tenancy migration created policies that did not bite — Prisma connects as `postgres`, which carries BYPASSRLS, so every policy was decorative. A second LOGIN role would fix that but costs a password in `.env.local` and in every deploy environment, a second pooler connection string, and a credential to rotate. SET ROLE reaches the same row-level outcome with none of that.
**Alternatives considered:** A separate LOGIN role for the application (deferred, not rejected — see the limitation below). Application-layer filtering alone (rejected — tenant isolation would depend on no query ever forgetting a scope filter, forever). RLS with `auth.uid()` in the Supabase idiom (rejected — Prisma connects straight to Postgres and never passes through PostgREST, so `auth.uid()` is never populated).
**Consequences:** Verified against the database rather than by reading policy: as tenant A, `SELECT` on tenants returns only A; tenant B fetched by its own id returns zero rows; each tenant sees 12 of 24 roles; with no context set, zero rows. Writes app_user must not make — updating roles, tenants or memberships, deleting roles or trades — all affect zero rows. The two writes the app needs do work: audit insert, and an identity amending its own row but not another's.

**The limitation, stated because it is easy to overclaim:** the privilege drop is **not one-way**. `SET ROLE` is authorised against the *session* user's memberships, and the session user is still the owner, so code that can issue raw SQL inside the callback can switch back. Tested and confirmed allowed. This defends against a query missing its tenant filter — the failure that actually happens — and not against deliberate escalation by code already running in the server process. A separate LOGIN role with no membership in the owner closes it. **That should be taken before there is a second real tenant**, and it is a connection-string change rather than a redesign, because `withTenant()` is the only thing that would need to point elsewhere.
